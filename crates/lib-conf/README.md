...
A new logging library, `crab-log`, wishes to expose some settings to its dependant crates.
...
Additionally, the library in this example has its [documentation](https://docs.rs/lib-conf-example-readme/latest) published to showcase
...
Otherwise, if `serde` is gated by a feature flag, you can add `lib-conf` with its default features,
...
- Provides out-of-the-box runtime configuration, bypassing the common need for binary-authors to
...
For example, to revert a field named `foo` to its library-defined default via a
